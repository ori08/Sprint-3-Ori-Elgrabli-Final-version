...
function renderEditor() {
    ...
    terms.forEach(term => {
        ...
        <div class="term">
          ...
          <button class="color-btn"  style="background-color: ${term.color}" onclick=" openColorSection(${chartIdx},this)"></button>
          <h2>${title} ${stringNum}</h2>
        <input id='term${chartIdx}-label' type="text" onkeyup=" onChangeTerm(this,${chartIdx})" data-trans="label" placeholder="Label">
        <input id='term${chartIdx}-value' type="text" onkeyup=" onChangeValue(this,${chartIdx})" data-trans="value" placeholder="Value">
 `
...
function resizeCanvas() {
    ...
    addEventListener('resize', (event) => {
        ...
        if (window.innerWidth > 1200) {
            ...
            gCanvas.height = (window.innerHeight / 3) 
            canvas_width = gCanvas.width
            gChart.title.x = canvas_width
...
function changePlaceholder(idx, key) {
    var terms = getTerms()
    var index = Math.round((idx / 2) - 0.1)
    var elTerm = document.getElementById(`term${index}-${key}`)
    elTerm.placeholder = terms[index][key]
}