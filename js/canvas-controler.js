...
// canvasWidth, canvasHeight, drawCount
var animetion_id

function createCanvas() {
    gCanvas = document.getElementById('canvas')
    gCtx = gCanvas.getContext('2d')
    if (window.innerWidth < 1200 && window.innerWidth > 950) gCanvas.height = window.innerHeight - 110
...
function drawRectingle(shape) {
    ...
    function animateRect() {
        if (isAnimationValid('rect', animateMode))
...
function drawArcPlusLine(shape) {
    ...
    function animateCircle() {
        if ((isAnimationValid('circle', animateMode)))
...
function drawRectDiagrm(shape) {
    ...
    animate()
    function animate() {
        if ((isAnimationValid('diagram', animateMode)))
...
function isAnimationValid(condition1, condition2) {
